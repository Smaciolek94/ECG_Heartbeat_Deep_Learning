#Version 3: I converted input into arrays
...
def Evaluator(Model):
    ...
    for i in range(0,5):
        xtrain, xtest, ytrain, ytest = sklearn.model_selection.train_test_split(total.iloc[:,0:187],total.iloc[:,187],test_size=0.20)
        Model.fit(xtrain, ytrain, batch_size=1, epochs=5, validation_split=0.2)
...
Layer2 = layers.MaxPooling1D(3)(Layer1)
...
#xtrain.reshape(11641, 187, 1)
Input = keras.Input(shape=(187,1))
Layer1 = layers.Conv1D(filters = 64,kernel_size = 3,activation='relu',
                       input_shape=(187,1))(Input)
Layer2 = layers.Flatten()(Layer1)
Layer3 = layers.Dense(64,activation="relu")(Layer2) #dimensionality error unless all dense layers after are 1
Layer4 = layers.Dense(64,activation="relu")(Layer3)
Output = layers.Dense(2)(Layer4)
